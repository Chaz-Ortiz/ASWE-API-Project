...
// if Search by Manufacturer button is selected
			...
			if (isset($_POST['submit']) && $_POST['submit'] == "search") {
				...
				$manuStr = ($manu == "all") ? "`manufacturer` LIKE '%'" : "`manufacturer` = '" . $dblink->real_escape_string($manu) . "'";

				// Query excludes any device that has been added to device_status_inactive
				$sql = "SELECT * FROM `devices` WHERE $manuStr AND `auto_id` NOT IN (SELECT `device_id` FROM `device_status_inactive`)";

				$result = $dblink->query($sql) or die("<h2>Something went wrong with $sql<br>" . $dblink->error . "</h2>");

				// Display results in a DataTable
				echo '<div class="row mt-4">';
				echo '<div class="col-md-8 offset-md-2">'; // center the table
